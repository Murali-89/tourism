...
import com.cts.tourism.tourismmanagement.entity.LoginRequestModel;
...
public class UserAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public UserAuthenticationFilter(UserServiceImpl userService,
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        UserEntity userDetails = userService.findByEMail(userName);
        String token = Jwts.builder()
                .setSubject(userDetails.getEmail())
                .setExpiration(new Date(System.currentTimeMillis() + Long.parseLong(environment.getProperty("token.expiration_time"))))
                .signWith(SignatureAlgorithm.HS512, environment.getProperty("token.secret") )
                .compact();
        String expTime = environment.getProperty("token.expiration_time");
